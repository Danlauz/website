#!/usr/bin/env python3
"""
Generate a single clean publication page from BibTeX files
Author: Dany Lauzon
Date: 2026-02-02
...
class APAPublicationGenerator:
    ...
    def format_entry_apa(self, entry):
        ...
        # Title
        title = self.clean_latex(entry.get('title', 'Sans titre'))
        ...
        if entry_type == 'article':
            ...
            thesis_type = entry.get('type', 'Thèse de doctorat')
    ...
    def generate_unified_page(self, bib_files, output_file="publications.qmd"):
        ...
        print("=" * 70)
        print("📚 Génération de la page unifiée des publications")
        print("=" * 70)
        print()
        ...
        for category, bib_file in bib_files.items():
            if not Path(bib_file).exists():
                print(f"⚠️  {bib_file} introuvable, ignoré...")
                continue
            
            print(f"📖 Lecture de {bib_file}...")
        ...
        output.append("---")
        output.append("")
        output.append(f"*Total: {total_count} publications | Dernière mise à jour: Février 2026*")
...
def main():
    ...
    bib_files = {
        'Articles de revue par les pairs': 'MyResearch_Article.bib',
        'Résumés et articles de conférence': 'MyResearch_Conference.bib',
        'Thèses': 'MyResearch_Thesis.bib',
        'Livres et ressources pédagogiques': 'MyResearch_Book.bib'
    }
    
    # Generate unified page
    generator.generate_unified_page(bib_files, output_file="publications.qmd")
    
    print("📋 Prochaines étapes:")
    print("  1. Vérifier le fichier publications.qmd généré")
    print("  2. L'ajouter à votre _quarto.yml si nécessaire")
    print("  3. Lancer 'quarto render' pour construire votre site")
    print()
